...
const SUPPORTED_COUNTRIES = [
    { value: 'South Africa', currencies: ['ZAR'], Icon: EnvironmentOutlined, detail: 'South African Rand' },
    { value: 'Zimbabwe', currencies: ['ZWL', 'ZAR', 'USD'], Icon: GlobalOutlined, detail: 'Zimbabwean Dollar' },
] as const
...
export default function ConsultantProfilePage() {
    ...
    const updateAvailabilitySlot = (id: string, change: Partial<{ startTime: string, endTime: string }>) => {
        ...
        if (start >= end) {
            message.warning('An availability window must end after it starts.')
            return
        }
        if (overlaps) {
            message.warning('That time overlaps another availability window for this day.')
            return
        }
    ...
    const save = async () => {
        ...
        if (!profile.country || !expectedPhoneLength) {
            message.error('Select South Africa or Zimbabwe before adding a primary phone number.')
            return
        }
        if (!localPhoneNumber(profile.phone)) {
            message.error('Primary phone is required.')
            return
        }
        ...
        try {
            ...
            message.success('Your consultant profile was saved.')
        } catch (error) {
            message.error(error instanceof Error ? error.message : 'Your profile could not be saved.')
        } finally {
            setSaving(false)
        }
    }
    ...
    const uploadImage = async (file: File) => {
        ...
        try {
            ...
            message.error(error instanceof Error ? error.message : 'Profile image could not be uploaded.')
        }
    }

    if (!profile) return <LoadingOverlay tip="Loading consultant profile" />
            ...
            <div className="consultant-profile-layout">
                <Card className="consultant-profile-identity-card">
                    <div className="consultant-profile-hero">
                        <div className="consultant-profile-photo">
                            <Upload accept="image/*" showUploadList={false} beforeUpload={(file) => { void uploadImage(file as File); return false }}>
                                <div className="consultant-profile-photo-target" role="button" tabIndex={0} aria-label="Change profile photo">
                        ...
                        <div className="consultant-profile-identity">
                            ...
                            {profile.verificationStatus === 'verified' && <Tag color="blue" icon={<SafetyCertificateOutlined />}>Verified consultant</Tag>}
                            {profile.status === 'published' && <Tag color="green">Published</Tag>}
                            <Typography.Paragraph type="secondary" className="consultant-profile-headline">{profile.headline || 'Add a professional headline to introduce your expertise.'}</Typography.Paragraph>
                        </div>
                    </div>
                    <div className="consultant-profile-facts" aria-label="Consultant profile summary">
                        <div className="consultant-profile-fact consultant-profile-fact-green"><MailOutlined /><span><small>Email</small><strong>{user?.email || 'Not added'}</strong></span></div>
                        <div className="consultant-profile-fact consultant-profile-fact-amber"><PhoneOutlined /><span><small>Phone</small><strong>{profile.phone || 'Not added'}</strong></span></div>
                        <div className="consultant-profile-fact consultant-profile-fact-teal"><SolutionOutlined /><span><small>Expertise</small><strong>{profile.specialties.length ? `${profile.specialties.length} area${profile.specialties.length === 1 ? '' : 's'}` : 'Not added'}</strong></span></div>
                        <div className="consultant-profile-fact consultant-profile-fact-rose"><EnvironmentOutlined /><span><small>Based in</small><strong>{[profile.province, profile.country].filter(Boolean).join(', ') || 'Not added'}</strong></span></div>
                    </div>
                    <div className="consultant-profile-acceptance">
                        ...
                        <span>{profile.acceptingClients ? 'Accepting new SME requests' : 'Not accepting new requests'}</span>
                        <Tag color={profile.status === 'published' ? 'green' : 'gold'}>{profile.status === 'published' ? 'Published' : 'Draft'}</Tag>
                    </div>
                </Card>
                <Card className="dashboard-section-card consultant-profile-editor-card">
                    <Form layout="vertical" requiredMark={false} className={`consultant-profile-form consultant-profile-form-${activeSection.toLowerCase()}`}>
                        <div className="consultant-profile-editor-top"><Segmented block value={activeSection} onChange={changeSection} options={[{ label: <span><SolutionOutlined /> Professional</span>, value: 'Professional' }, { label: <span><EnvironmentOutlined /> Location</span>, value: 'Location' }, { label: <span><IdcardOutlined /> Contact</span>, value: 'Contact' }, { label: <span><EnvironmentOutlined /> Availability</span>, value: 'Availability' }]} /></div>
                        {activeSection === 'Professional' && <div className="consultant-inline-services"><div className="consultant-inline-services-head"><div><Typography.Title level={4}>Services & rates</Typography.Title><Typography.Paragraph type="secondary">Add services using the expertise areas above.</Typography.Paragraph></div><Button icon={<PlusOutlined />} disabled={!profile.specialties.length} onClick={addService}>Add service</Button></div>{profile.services.map((service) => <Card key={service.id} id={`consultant-service-${service.id}`} size="small" className="consultant-inline-service-card"><div className="consultant-inline-service-head"><Typography.Text strong>{service.areaOfSupport || 'New service'}</Typography.Text><Button danger type="text" icon={<DeleteOutlined />} onClick={() => update({ services: profile.services.filter((item) => item.id !== service.id) })} /></div><Row gutter={[10, 8]}><Col xs={24} md={10}><Select style={{ width: '100%' }} placeholder="Area of support" value={service.areaOfSupport || undefined} options={profile.specialties.map((value) => ({ value, label: value }))} onChange={(value) => updateService(service.id, { areaOfSupport: value })} /></Col><Col xs={24} md={14}><Select mode="tags" style={{ width: '100%' }} placeholder="Specific interventions" value={service.interventionExamples} onChange={(value) => updateService(service.id, { interventionExamples: value })} /></Col><Col xs={24} md={8}><Select style={{ width: '100%' }} value={service.deliveryMode} options={[{ value: 'online', label: 'Online' }, { value: 'in_person', label: 'In person' }, { value: 'hybrid', label: 'Hybrid' }]} onChange={(value) => updateService(service.id, { deliveryMode: value })} /></Col><Col xs={24} md={8}><InputNumber min={0} value={service.rate} style={{ width: '100%' }} addonAfter={profile.currency || 'rate'} onChange={(value) => updateService(service.id, { rate: Number(value) || 0 })} /></Col><Col xs={24} md={8}><Select style={{ width: '100%' }} value={service.rateUnit} options={[{ value: 'day', label: 'Per day' }]} onChange={(value) => updateService(service.id, { rateUnit: value })} /></Col></Row></Card>)}</div>}
                        {activeSection === 'Availability' && <div className="consultant-availability-editor">
                            <div className="consultant-special-availability">
                                <div className="consultant-special-availability-row"><div><strong>Weekend availability</strong><small>Set your usual Saturday and Sunday availability before adding any weekend time windows.</small></div><div className="consultant-special-options" role="radiogroup" aria-label="Weekend availability">{SPECIAL_AVAILABILITY_OPTIONS.map(({ value, label }) => <button type="button" key={value} role="radio" aria-checked={profile.weekendAvailability === value} className={profile.weekendAvailability === value ? 'is-selected' : ''} onClick={() => setWeekendAvailability(value)}>{label}</button>)}</div></div>
                            </div>
                            <div className="consultant-weekday-grid" role="tablist" aria-label="Select a weekday">
                        ...
                        <Row className="consultant-profile-fields" id="consultant-section-professional" gutter={[16, 8]}>
                            <Col className="consultant-field-professional" xs={24} md={12}>
                                <Form.Item label="Professional headline">
                                    <Input value={profile.headline} maxLength={100} placeholder="e.g. SME finance and growth strategy specialist" onChange={(event) => update({ headline: event.target.value })} />
                                </Form.Item>
                            </Col>
                            <Col className="consultant-field-professional" xs={24} md={12}>
                            ...
                            <Col className="consultant-field-professional" xs={24}>
                                <Form.Item label="About your work">
                                    <Input.TextArea rows={5} maxLength={800} showCount value={profile.bio} placeholder="Summarise the outcomes you help SMEs achieve." onChange={(event) => update({ bio: event.target.value })} />
                                </Form.Item>
                            </Col>
                            <Col className="consultant-field-professional" xs={24}>
                                <Form.Item label="Areas of expertise">
                                    <Select mode="tags" value={profile.specialties} tokenSeparators={[',']} placeholder="Finance, marketing, operations..." onChange={(value) => update({ specialties: value })} />
                                </Form.Item>
                            </Col>
                            ...
                            <Col className="consultant-field-location" xs={24} md={12}>
                                <Form.Item label="Province / region" required>
                                    <Select value={profile.province || undefined} placeholder="Select a province or region" disabled={!profile.country} options={provincesForCountry(profile.country).map((value) => ({ value, label: value }))} onChange={(province) => update({ province })} />
                                </Form.Item>
                            </Col>
                            <Col className="consultant-field-location" xs={24} md={12}>
                                <Form.Item label="Physical address" required>
                                    <Input value={profile.physicalAddress || profile.operatingLocation} placeholder="Street, suburb, town/city" onChange={(event) => update({ physicalAddress: event.target.value, operatingLocation: event.target.value })} />
                                </Form.Item>
                            </Col>
                            <Col className="consultant-field-location" xs={24} md={12}>
                                <Form.Item label="In-person radius">
                                    <div className="consultant-radius-choice-grid" role="radiogroup" aria-label="In-person radius">
                                        {[10, 25, 50, 100, 250].map((radius) => <button key={radius} type="button" role="radio" aria-checked={profile.serviceRadiusKm === radius} className={`consultant-profile-choice-card${profile.serviceRadiusKm === radius ? ' is-selected' : ''}`} onClick={() => update({ serviceRadiusKm: radius })}><AimOutlined /><span><strong>{radius === 250 ? '250+ km' : `${radius} km`}</strong><small>{radius <= 25 ? 'Nearby' : radius <= 100 ? 'Regional' : 'Wide reach'}</small></span></button>)}
                                    </div>
                                </Form.Item>
                            </Col>
                            <Col className="consultant-field-location" xs={24} md={12}>
                                <Form.Item label="Daily-rate currency" required>
                                    <div className="consultant-location-choice-grid consultant-currency-choice-grid" role="radiogroup"
                                        ...
                                        {!SUPPORTED_COUNTRIES.some(({ value }) => value === profile.country) && <div className="consultant-profile-choice-empty">Choose a country to set your currency.</div>}
                                    </div>
                                </Form.Item>
                            </Col>
                        </Row>
                        <Row id="consultant-section-contact" className="consultant-contact-fields" gutter={[16, 8]}>
                            <Col xs={24}>
                                <div className="consultant-contact-email-card"><MailOutlined /><div><small>System login email</small><strong>{profile.email || user?.email || 'Not added'}</strong></div><Tag color="blue">Account</Tag></div>
                            </Col>
                                <Col xs={24}>
                                    <Form.Item label="Additional email">
                                        <Input prefix={<MailOutlined />} type="email" value={profile.alternativeEmail} placeholder="Optional work or personal email" onChange={(event) => update({ alternativeEmail: event.target.value })} />
                                    </Form.Item>
                                </Col>
                                <Col xs={24} md={12}>
                                    <Form.Item label="Primary phone" required validateStatus={profile.phone && !primaryPhoneIsValid ? 'error' : undefined} help={profile.phone && !primaryPhoneIsValid ? `Enter ${expectedPhoneLength || 9} digits after ${diallingCode || 'the country code'}.` : undefined}>
                                        <div className="consultant-phone-input-row"><Input addonBefore={diallingCode || '—'} disabled={!diallingCode} maxLength={11} value={formatLocalPhoneNumber(profile.phone)} placeholder="78 571 9132" onChange={(event) => update({ phone: withDiallingCode(event.target.value, profile.country) })} /><Checkbox checked={profile.phoneIsWhatsApp} disabled={!profile.phone.trim()} onChange={(event) => update({ phoneIsWhatsApp: event.target.checked })}>WhatsApp</Checkbox></div>
                                    </Form.Item>
                                </Col>
                                <Col xs={24} md={12}>
                                    <Form.Item label="Alternative phone" validateStatus={profile.alternativePhone && !alternativePhoneIsValid ? 'error' : undefined} help={profile.alternativePhone && !alternativePhoneIsValid ? `Enter ${expectedPhoneLength || 9} digits after ${diallingCode || 'the country code'}.` : undefined}>
                                        <div className="consultant-phone-input-row"><Input addonBefore={diallingCode || '—'} disabled={!diallingCode} maxLength={11} value={formatLocalPhoneNumber(profile.alternativePhone)} placeholder="78 571 9132" onChange={(event) => update({ alternativePhone: withDiallingCode(event.target.value, profile.country) })} /><Checkbox checked={profile.alternativePhoneIsWhatsApp} disabled={!profile.alternativePhone.trim()} onChange={(event) => update({ alternativePhoneIsWhatsApp: event.target.checked })}>WhatsApp</Checkbox></div>
                                    </Form.Item>
                            </Col>
                            <Col xs={24} md={12}>
                        ...
                        <Button className="consultant-profile-save" type="primary" icon={<SaveOutlined />} loading={saving} onClick={() => void save()}>Save profile</Button>
                    </Form>
                </Card>
            </div>
        </DashboardPage>
